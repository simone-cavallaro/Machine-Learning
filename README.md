This is the folder for the Machine Learning course. We started the course on 02/02/2026.
- 1st lab session (Anna) (05/02/2026) --> we used the fuel consumpution rating dataset to perform linear regression;
- 2nd lab session (Riccardo) (11/02/2026) --> we used house dataset (of sklearn) to perform EDA;
- 3rd lab session (Anna) (12/02/2026) --> imputation of missing values;
- 4th lab session (Riccardo) (16/02/2026) --> we finished the 2nd lab session file and starting the lab 5 on Classification & Clustering Lab;
- 5th lab session (Anna) (19/02/2026) --> we created our first neural network on the MNIST dataset;
- 6th lab session (Anna) (26/02/2026) --> we saw what happens inside the black box (the hidden layers);